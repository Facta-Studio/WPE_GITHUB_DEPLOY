...
LOCAL_BRANCH_DEFAULT="staging"
BRANCH=${LOCAL_BRANCH:-$LOCAL_BRANCH_DEFAULT}

mkdir "$SSH_PATH"

ssh-keyscan -t rsa "TEST" >> "$KNOWN_HOSTS_PATH"
...
git config core.sshCommand "ssh -i $WPENGINE_SSH_KEY_PUBLIC_PATH -o UserKnownHostsFile=$KNOWN_HOSTS_PATH"
